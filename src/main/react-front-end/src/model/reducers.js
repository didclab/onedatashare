...
export function onedatashareModel(state = initialState, action) {
  ...
  switch (action.type) {
    case LOGIN:
      ...
      cookies.set('email', email, { maxAge : expiresIn, domain: process.env.ODS_APP_DOMAIN, secure: true, sameSite: 'None' });
      cookies.set('saveOAuthTokens', saveOAuthTokens, { maxAge : expiresIn, domain: process.env.ODS_APP_DOMAIN, secure: true, sameSite: 'None'});
      cookies.set('compactViewEnabled', compactViewEnabled, { maxAge : expiresIn, domain: process.env.ODS_APP_DOMAIN, secure: true, sameSite: 'None' });
