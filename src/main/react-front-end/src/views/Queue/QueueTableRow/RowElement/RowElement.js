...
export default class RowElement extends React.Component {
  infoRow() {
    return <InfoRow resp={this.props.resp} span={this.props.adminPg ? 9 : 7} />;
  }
  ...
  render() {
    ...
    let bar = (
      <QueueProgressBar
        status={resp.status}
        total={resp.jobParameters.jobSize}
        done={resp.jobParameters.jobSize}
      />
    );
    ...
    let difference =
      (Date.parse(resp.endTime) - Date.parse(resp.startTime)) / 1000;
    let speed =
      parseFloat((resp.jobParameters.jobSize / 1000000) * 8) / difference;
    if (isNaN(speed)) {
      speed = 0;
    }
      ...
      <React.Fragment>
        <TableRow className={"QueueRow"} style={{ alignSelf: "stretch" }}>
          <Hidden mdDown>
            ...
            <TableCell
              className={"idCell" + (admin ? "-admin" : "") + " queueBodyCell"}
            >
              <p>{resp.id}</p>
            </TableCell>
            <TableCell
              className={
                "progressCell" + (admin ? "-admin" : "") + " queueBodyCell"
              }
            >
              {bar}
            </TableCell>
            <TableCell
              className={
                "speedCell" + (admin ? "-admin" : "") + " queueBodyCell"
              }
            >
            ...
            <TableCell
              className={
                "sourceCell" + (admin ? "-admin" : "") + " queueBodyCell"
              }
            >
            ...
            <TableCell
              className={
                "destinationCell" + (admin ? "-admin" : "") + " queueBodyCell"
              }
            >
            ...
            <TableCell
              className={
                "actionCell" + (admin ? "-admin" : "") + " queueBodyCell"
              }
            >
              {actions}
            </TableCell>
          ...
          <Hidden lgUp>
            <TableCell className="mobileCell">
              ...
              <p>
                <b>Average Speed:</b> {humanReadableSpeed(speed)}
              </p>
              <p>
                <b>Source:</b> {resp.jobParameters.sourceBasePath}
              </p>
              <p>
                <b>Destination:</b> {resp.jobParameters.destBasePath}
              </p>
              <p>{time}</p>
              {actions}
            </TableCell>
