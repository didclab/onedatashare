...
function makeHeaderCells(
  ...
  let keys = [
    sortableColumns.jobId,
    sortableColumns.status,
    sortableColumns.avgSpeed,
    sortableColumns.source,
    sortableColumns.destination,
  ];
  if (adminPg) {
  ...
  for (let i = 0; i < titles.length; i += 1) {
    ...
    menuOpts.push(<MenuItem value={keys[i]}>{titles[i]}</MenuItem>);
  }
  ...
  return (
    <TableHead>
      ...
      <TableRow>
        <Hidden mdDown>
          {headerCells}
          <TableCell className="actionCell queueHeaderCell">
            <p>Actions</p>
          </TableCell>
        </Hidden>
        <Hidden lgUp>
