...
class SentMail extends Component {
  ...
  handleCheckBoxClick = (event, type, email) => {
    ...
    if (type === "selected") {
      this.setState({
        checkedSelectedUsers: newList,
        selectAllCheckedSUsers:
          newList.length === this.state.selectedUsers.length,
      });
    } else {
      this.setState({
        checkedUnSelectedUsers: newList,
        selectAllCheckedUSUsers:
          newList.length === this.state.unSelectedUsers.length,
      });
    }
  };
  ...
  handleSelectAllClick = (event, type) => {
    ...
    if (type === "selected") {
      if (event.target.checked) {
        this.setState({
          checkedSelectedUsers: selectedUsers,
          selectAllCheckedSUsers: !selectAllCheckedSUsers,
        });
      } else {
        this.setState({
          checkedSelectedUsers: [],
      ...
      if (event.target.checked) {
        this.setState({
          checkedUnSelectedUsers: unSelectedUsers,
          selectAllCheckedUSUsers: !selectAllCheckedUSUsers,
        });
      } else {
        this.setState({
          checkedUnSelectedUsers: [],
  ...
  moveToUnselectedList = () => {
    ...
    const newSelectedUsers = selectedUsers.filter(
      (user) => !this.state.checkedSelectedUsers.includes(user)
    );
    const newUnSelectedUsers = unSelectedUsers.concat(checkedSelectedUsers);
    this.setState({
      selectedUsers: newSelectedUsers,
      checkedSelectedUsers: [],
      unSelectedUsers: newUnSelectedUsers,
    });
  };

  moveToSelectedList = () => {
    const { checkedUnSelectedUsers, selectedUsers, unSelectedUsers } =
      this.state;
    const newUnSelectedUsers = unSelectedUsers.filter(
      (user) => !this.state.checkedUnSelectedUsers.includes(user)
    );
    const newSelectedUsers = selectedUsers.concat(checkedUnSelectedUsers);
    const showSUErrorMsg =
      newSelectedUsers.length === 0
        ? "Please select atleast 1 user to send email."
        : null;
    this.setState({
      showSUErrorMsg,
      selectedUsers: newSelectedUsers,
  ...
  deleteMail = async () => {
    ...
    if (result.status === 200) {
      this.setState({
        ...
        showDelete: false,
        successMsg: "Mail Deleted Successfully.",
      });
      await this.getSentMails();
    } else {
      this.setState({
  ...
  onSubjectChange = (event) => {
    this.setState({
      currentView: { ...this.state.currentView, subject: event.target.value },
    });
  };
  ...
  showSelectUsersOpen = () => {
    ...
    if (
      ...
      this.state.currentView.recipients &&
      this.state.filteredUsers
    ) {
      unSelectedUsers = this.state.filteredUsers.filter(
        (user) => !this.state.currentView.recipients.includes(user.email)
      );
    }
    ...
    this.setState({
      ...
      showSUErrorMsg: null,
      unSelectedUsers,
      selectedUsers: this.state.currentView.recipients,
    });
  };

  onMessageChange = (event) => {
    this.setState({
      currentView: { ...this.state.currentView, message: event.target.value },
    });
  };
  ...
  onSelectUsers = () => {
    ...
    if (selectedUsers.length === 0) {
      this.setState({
        showSUErrorMsg: "Please select atleast 1 user to send email.",
      });
    } else {
      this.setState({
        showSelectUsers: false,
        currentView: { ...this.state.currentView, recipients: selectedUsers },
      });
    }
  };
  ...
  render() {
    ...
    const mails = this.state.mails
      ...
      .reverse();
    const tooltip = this.state.currentView ? (
      <Tooltip id="tooltip">
        {this.state.currentView.recipients &&
          this.state.currentView.recipients.map((mail, index) => {
            return (
              <p>
                {mail}
                {"\n"}
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column" }}>
        ...
        >
          ...
          {showErrorChip ? (
            <Alert bsStyle="danger" style={styles.alertStyle}>
              <p>{errorMsg}</p>
              <button class="close" onClick={this.closeAlert}>
                x
              </button>
            </Alert>
          ) : (
                ...
                {mails.map((mail, index) => {
                  return (
                    <tr>
                      <td>{index + 1}</td>
                      <td>
                        {new Date(mail.sentDateTime).toLocaleDateString()}
                      </td>
                      <td>
                        {mail.recipients.length === 1
                          ? "1 user"
                          : `${mail.recipients.length} users`}
                      </td>
                      <td>{mail.subject}</td>
                      ...
                      <td>
                        <ButtonToolbar>
                          ...
                          >
                            <Glyphicon
                              glyph="edit"
                              style={{ color: "teal", fontSize: 18, margin: 7 }}
                          ...
                          >
                            <Glyphicon
                              glyph="trash"
                              style={{ color: "red", fontSize: 18, margin: 7 }}
                              onClick={() => this.showDelete(mail)}
                            />
                          </OverlayTrigger>
                        </ButtonToolbar>
                      </td>
          ...
          >
            <Modal.Header closeButton>
              <Modal.Title id="contained-modal-title-lg">
                <Glyphicon glyph="envelope" /> Mail Box
              </Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <Form>
                <FormGroup
                  validationState={isValidRecipients ? "none" : "error"}
                >
                  ...
                  <OverlayTrigger placement="right" overlay={tooltip}>
                    <FormControl.Static style={{ width: "15%" }}>
                      {this.state.currentView &&
                      this.state.currentView.recipients &&
                      this.state.currentView.recipients.length > 0 ? (
                        <Link
                          href="#"
                          onClick={this.showSelectUsersOpen}
                        >{`${this.state.currentView.recipients.length} users selected`}</Link>
                      ) : (
                        <Link href="#">No recipients </Link>
                      )}
                    </FormControl.Static>
                  </OverlayTrigger>
                  {isValidRecipients ? (
                    ...
                    <HelpBlock>{"Recipients cannot be empty."}</HelpBlock>
                  )}
                </FormGroup>
                <FormGroup
                  controlId={"subject"}
                  validationState={isValidSubject ? "none" : "error"}
                >
                  ...
                  {isValidSubject ? (
                    ...
                    <HelpBlock>{"Please enter a valid subject."}</HelpBlock>
                  )}
                </FormGroup>
                <FormGroup
                  controlId={"message"}
                  validationState={isValidMessage ? "none" : "error"}
                >
                  ...
                  <textarea
                    ...
                    rows="10"
                    value={this.state.currentView.message}
                    onChange={this.onMessageChange}
                  ></textarea>
                  {isValidMessage ? (
                    ""
                  ) : (
                    <HelpBlock>{"Please enter a valid message."}</HelpBlock>
                  )}
                </FormGroup>
              </Form>
            </Modal.Body>
            <Modal.Footer>
              <Button bsStyle="success" onClick={this.onResend}>
                Resend
              </Button>
        ...
        >
              ...
              <div style={{ display: "flex", justifyContent: "space-around" }}>
                      ...
                      <tr>
                        <th>
                          ...
                          >
                            <Checkbox
                              checked={
                                this.state.selectAllCheckedSUsers &&
                                this.state.selectedUsers.length > 0 &&
                                this.state.selectedUsers.length ===
                                  this.state.checkedSelectedUsers.length
                              }
                              style={{ margin: 0 }}
                              onChange={(event) =>
                                this.handleSelectAllClick(event, "selected")
                              }
                            ></Checkbox>
                          </FormGroup>
                        </th>
                ...
                <Col lg={2}>
                  ...
                  >
                    <Button
                      disabled={
                        !this.state.checkedSelectedUsers ||
                        this.state.checkedSelectedUsers.length === 0
                      }
                      onClick={this.moveToUnselectedList}
                    >
                      {" "}
                      {">"}{" "}
                    </Button>
                    <Button
                      disabled={
                        !this.state.checkedUnSelectedUsers ||
                        this.state.checkedUnSelectedUsers.length === 0
                      }
                      onClick={this.moveToSelectedList}
                    >
                      {" "}
                      {"<"}{" "}
                    </Button>
                      ...
                      <tr>
                        <th>
                          ...
                          >
                            <Checkbox
                              checked={
                                this.state.selectAllCheckedUSUsers &&
                                this.state.unSelectedUsers.length > 0 &&
                                this.state.checkedUnSelectedUsers.length ===
                                  this.state.unSelectedUsers.length
                              }
                              style={{ margin: 0 }}
                              onChange={(event) =>
                                this.handleSelectAllClick(event, "unselected")
                              }
                            ></Checkbox>
                          </FormGroup>
                        </th>
          ...
          <Modal.Footer>
            <Button bsStyle={"primary"} onClick={this.onSelectUsers}>
              Select
            </Button>
        ...
        >
          ...
          <Modal.Footer>
            <Button bsStyle="danger" onClick={this.deleteMail}>
              Yes
            </Button>
