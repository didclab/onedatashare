...
class ComposeMail extends Component {
  ...
  handleFilter = (event) => {
    ...
    const filteredUsers = users
      .filter(
        (user) =>
          user.firstName.indexOf(event.target.value) > -1 ||
          user.lastName.indexOf(event.target.value) > -1 ||
          user.email.indexOf(event.target.value) > -1
      )
      .map((user) => user);
  ...
  handleDialogBox = () => {
    this.externalWindow = window.open(
      "",
      "",
      "width=750,height=550,left=150,top=150"
    );
      ...
      <div>
        ...
        <Form>
            ...
            <FormControl.Static>
              {selected && selected.length > 0 ? (
                <Link
                  href="#"
                  onClick={this.smOpen}
                >{`${selected.length} users selected`}</Link>
              ) : (
                <Link href="#" onClick={this.smOpen}>
                  Select recipients{" "}
                </Link>
              )}
            </FormControl.Static>
          ...
          >
            <FormGroup controlId={"isHtml"}>
              <Checkbox
                ...
                style={{ margin: 0 }}
                onChange={this.handleisHtml}
              >
                <b>Sending HTML Mail?</b>
              </Checkbox>
            </FormGroup>
            {this.state.ishtml ? (
              <Link href="#" onClick={this.handleDialogBox}>
                Preview
              </Link>
            ) : (
              <div></div>
          ...
          <ButtonToolbar style={styles.ButtonToolbar}>
            <Button bsStyle="default" onClick={this.onClear}>
              Clear
            </Button>
            <Button
              bsStyle="primary"
        ...
        >
          <Modal.Header style={styles.headerStyle}>
            <div style={{ flex: 1 }}>
              <h4 bsStyle="primary" style={styles.modalHeading}>
                Select Users
              </h4>
            </div>
            <div style={{ flex: 2 }}>
            ...
            <div style={{ flex: 1, alignItems: "flex-end" }}>
              <button class="close" onClick={this.smClose}>
                x
              </button>
            </div>
          </Modal.Header>
                            ...
                            <td>
                              ...
                              >
                                <Checkbox
                                  ...
                                  style={{ margin: 0 }}
                                  onChange={(event) =>
                                    this.handleClick(event, user.email)
                                  }
                                ></Checkbox>
                              </FormGroup>
                            </td>
