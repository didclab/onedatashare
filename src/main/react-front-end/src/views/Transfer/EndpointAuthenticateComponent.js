...
import {
  ...
  globusEndpointDetail,
  deleteEndpointId,
  globusEndpointActivateWeb,
} from "../../APICalls/globusAPICalls";
import {
  savedCredList,
  saveEndpointCred,
  deleteCredential,
...
import DataIcon from "@mui/icons-material/Laptop";
...
export default class EndpointAuthenticateComponent extends Component {
  ...
  constructor(props) {
    ...
    if (
      !isOAuth[
        loginType
      ] /*loginType === FTP_TYPE || loginType === SFTP_TYPE || loginType === HTTP_TYPE*/
    ) {
    ...
    this.getEndpointListComponentFromList =
      this.getEndpointListComponentFromList.bind(this);
  ...
  historyListUpdateFromBackend = (endpointType) => {
    savedCredList(
      ...
      (error) => {
        this._handleError(
          "Unable to retrieve data from backend. Try log out or wait for few minutes."
        );
        this.props.setLoading(false);
      }
    );
  ...
  handleUrlChange = (event) => {
    ...
    this.setState({
      portNumField:
        colonCount < 2 ||
        getType(this.state.endpoint) === showType.s3 /*&& colonCount<3*/,
      url: url,
    });
  };
  ...
  endpointCheckin = (url, portNum, credential, callback) => {
    ...
    if (
      getType(this.state.endpoint) !== showType.s3 &&
      !getTypeFromUri(endpointSet.uri)
    ) {
  ...
  getEndpointListComponentFromList(endpointIdsList) {
    return Object.keys(endpointIdsList).map((v) => (
      <ListItem
        button
        key={v}
        onClick={() => {
          globusEndpointDetail(
            endpointIdsList[v].id,
            (resp) => {
      ...
      >
        ...
        <ListItemText
          primary={endpointIdsList[v].name}
          secondary={endpointIdsList[v].canonical_name}
        />
          ...
          >
            ...
            <ListItemSecondaryAction>
              <IconButton
                aria-label="Delete"
                onClick={() => {
                  let endPointType = Object.keys(showType).find(
                    (key) => showType[key] === this.state.endpoint.uri
                  );
                  deleteCredential(
                    endPointType,
                    v,
  ...
  getHistoryListComponentFromList(historyList) {
    return historyList.map((uri) => (
      <ListItem
        ...
        onClick={() => {
          if (
            showDisplay[getName(this.state.endpoint).toLowerCase()].label ===
            showDisplay.s3.label
          ) {
            ...
            listFiles(
              ...
              (error) => {
                ...
                this.setState({
                  url: uri,
                  authFunction: this.regularSignIn,
                  settingAuth: true,
                  needPassword: true,
                  portNum: region,
                });
            ...
            let endpointSet = {
              ...
              credential: {
                name: '""',
                credId: uri,
                type: getName(this.state.endpoint).toLowerCase(),
              },
        ...
        <ListItemSecondaryAction>
          <IconButton
            ...
            onClick={() => {
              ...
              if (
                showDisplay[getName(this.state.endpoint).toLowerCase()]
                  .label === showDisplay.s3.label
              ) {
                deleteCredential(
                  endPointType,
                  uri,
  ...
  regularSignIn = () => {
    ...
    if (
      (url.substr(url.length - 3) === "://" && loginType !== showType.s3) ||
      (url.length < 1 && this.state.portNum < 1)
    ) {
      loginType !== showType.s3
        ? this._handleError("Please enter a valid URL")
        : this._handleError("Please enter a valid bucketname and region");
      return;
    }
    // User is expected to enter password to login
    if (
      (!typesWithOptionalUserName.includes(loginType) &&
        username.length === 0) ||
      (!typesWithOptionalPassword.includes(loginType) && password.length === 0)
    ) {
      this._handleError(
        loginType !== showType.s3
          ? "Enter a username or password"
          : "Enter an access key or secret key"
      );
      return;
    }
  ...
  endpointModalLogin = (endpoint) => {
    if (endpoint.activated === "false") {
      eventEmitter.emit(
        "messageOccured",
        "Please activate your globus endpoint using credential on the new tab"
      );
      ...
      this.endpointCheckin(
        "gsiftp:///",
        this.state.portNum,
        { type: "globus", globusEndpoint: endpoint },
        (msg) => {
          this._handleError("Authentication Failed");
        }
      );
  ...
  deleteConfirmationModal = () => {
    ...
    return (
      <Dialog open={this.state.openModal} onClose={handleClose}>
        <DialogContent>
          <DialogContentText>
      ...
      <div>
        {!settingAuth && (
          <div className={"authenticationContainer"}>
            {!(loginType === showType.vfs) && (
              <ListItem
                id={endpoint.side + "Add"}
                button
                onClick={() => {
                  if (isOAuth[loginType] && loginType !== showType.gsiftp) {
                    //check if OAuth protocol
                    OAuthFunctions[loginType]();
                  } else if (loginType === showType.gsiftp) {
                    //check if globus protocol
                    this.setState({
                      selectingEndpoint: true,
                      authFunction: this.globusSignIn,
                    });
                  } else {
                    let loginUri =
                      getType(endpoint) === showType.s3 ? "" : loginType;
                    this.setState({
                      settingAuth: true,
                      authFunction: this.regularSignIn,
                      needPassword: false,
                      url: loginUri,
                      portNum: getDefaultPortFromUri(loginUri),
                    });
                  }
                }}
              >
                ...
                <ListItemText
                  primary={"Add New " + showDisplay[type.toLowerCase()].label}
                />
              </ListItem>
            )}
            ...
            {isOAuth[loginType] &&
              ...
              this.getCredentialListComponentFromList(
                credList,
                type,
                loginType
              )}
        ...
        >
          <GlobusEndpointListingComponent
            close={endpointModalClose}
            endpointAdded={this.endpointModalAdd}
          />
        </Modal>
        {settingAuth && (
          <div className={"authenticationContainer"}>
            <div style={{ paddingLeft: "3%", paddingRight: "3%" }}>
              ...
              >
                <TextValidator
                  ...
                  label={
                    loginType === showType.s3 ? "AWS ACCESS KEY" : "Username"
                  }
                ...
                <TextValidator
                  ...
                  label={
                    loginType === showType.s3 ? "AWS SECRET KEY" : "Password"
                  }
                  type="password"
              ...
              <div style={{ paddingLeft: "3%", paddingRight: "3%" }}>
                ...
                >
                  ...
                  <TextValidator
                    ...
                    style={{
                      width: "20%",
                      background: this.state.portNumField ? "white" : "#D3D3D3",
                    }}
