...
export default class CompactFileNodeWrapper extends Component {
    ...
    return (
      <div
        ...
        onMouseDown={(e) => {
          ...
          let rect = document
            .getElementById("browser" + this.props.endpoint.side)
            .getBoundingClientRect();
          this.setState({
            showBar: targetNum + 1,
            leftPosition: e.clientX - rect.left,
          });
        }}
      ></div>
    );
  ...
  componentDidMount() {
    ...
    this.setState({
      headerCompactStylePos: headerStyleDelta,
      compactStylePos: headerStyleDelta,
    });
  ...
  contextMenu(e) {
    ...
    this.offset =
      e.pageX - this.refs.CompactHeader.getBoundingClientRect().left;
    return false;
  }

  onMouseUp = (e) => {
    this.setState({
      compactStylePos: this.state.headerCompactStylePos.slice(),
      showBar: null,
    });
    ...
    return (
      <div ref={this.CompactTable}>
        <table id={"tableHeader" + this.props.endpoint.side}>
          <thead
            style={{ boxShadow: "0px 0px 5px #aaaaaa", overflow: "visible" }}
