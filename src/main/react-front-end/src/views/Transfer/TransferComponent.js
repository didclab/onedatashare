...
import {
  ...
  setDraggingTask,
  getEntities,
  setBeforeTransferReorder,
  getEndpointFromColumn,
  getSelectedTasksFromSide,
  longestCommonPrefix,
} from "./initialize_dnd.js";
...
export default class TransferComponent extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      history: [],
      settings: {
        optimizer: localStorage.hasOwnProperty("optimizer")
          ? localStorage.getItem("optimizer")
          : "None",
        overwrite: localStorage.hasOwnProperty("overwrite")
          ? JSON.parse(localStorage.getItem("overwrite"))
          : true,
        verify: localStorage.hasOwnProperty("verify")
          ? JSON.parse(localStorage.getItem("verify"))
          : true,
        encrypt: localStorage.hasOwnProperty("encrypt")
          ? JSON.parse(localStorage.getItem("encrypt"))
          : true,
        compress: localStorage.hasOwnProperty("compress")
          ? JSON.parse(localStorage.getItem("compress"))
          : true,
        retry: localStorage.hasOwnProperty("retry")
          ? Number(localStorage.getItem("retry"))
          : 5,
        concurrencyThreadCount: localStorage.hasOwnProperty(
          "concurrencyThreadCount"
        )
          ? Number(localStorage.getItem("concurrencyThreadCount"))
          : 1,
        pipeSize: localStorage.hasOwnProperty("pipeSize")
          ? Number(localStorage.getItem("pipeSize"))
          : 1,
        chunkSize: localStorage.hasOwnProperty("chunkSize")
          ? Number(localStorage.getItem("chunkSize"))
          : 10400000,
        parallelThreadCount: localStorage.hasOwnProperty("parallelThreadCount")
          ? Number(localStorage.getItem("parallelThreadCount"))
          : 1,
  ...
  setDate = (new_date) => {
    ...
    this.setState({
      settings: { ...this.state.settings, scheduledTime: iso8601_conversion },
    });
  };
  ...
  sendFile = (processed) => {
    ...
    let sType = formatType(
      endpointSrc.credential.type != null
        ? endpointSrc.credential.type
        : getType(endpointSrc)
    );
    let dType = formatType(
      endpointDest.credential.type != null
        ? endpointDest.credential.type
        : getType(endpointDest)
    );
    ...
    if (isOAuth[showType[sType]]) {
      ...
      sourceParent = Array.isArray(processed.fromTo[0].path)
        ? ""
        : processed.fromTo[0].path;
      processed.selectedTasks.forEach((x) => {
        infoList.push({ path: x.value });
      });
    } else if (sType === "http") {
      sourceParent = longestCommonPrefix(
        processed.fromTo[0].selectedTasks.map((x) => x.id)
      );
      ...
      sourceCredId = endpointSrc.credential.credId;
      processed.selectedTasks.forEach((x) =>
        infoList.push({ id: x.name, size: x.size, path: x.id })
      );
    } else {
      sourceParent = longestCommonPrefix(
        processed.fromTo[0].selectedTasks.map((x) => x.id)
      );
      sourceParent = sourceParent.includes(".")
        ? sourceParent.substr(0, sourceParent.lastIndexOf("/")) +
          (sourceParent !== "")
          ? ""
          : "/"
        : sourceParent;
    ...
    if (isOAuth[showType[dType]]) {
      ...
      destParent = Array.isArray(processed.fromTo[1].path)
        ? ""
        : processed.fromTo[1].path;
      destCredId = endpointDest?.credential?.credId;
    } else {
      destParent = longestCommonPrefix(
        processed.fromTo[1].selectedTasks.map((x) => x.id)
      );
      destParent = destParent.includes(".")
        ? destParent.substr(0, destParent.lastIndexOf("/")) + "/"
        : destParent;
      destCredId = endpointDest.credential.credId;
    }
    let source = {
    ...
    submitTransferRequest(
      ...
      (response) => {
        eventEmitter.emit(
          "messageOccured",
          "Transfer initiated! Please visit the queue page to monitor the transfer"
        );
  ...
  updateBrowseOne(object) {
    ...
    this.setState({
      endpoint1: object.endpoint || this.state.endpoint1,
      mode1: object.mode,
    });
    if (object.endpoint)
      store.dispatch(
        endpointUpdate("left", { ...this.state.endpoint1, ...object.endpoint })
      );
  }
  ...
  updateBrowseTwo(object) {
    ...
    this.setState({
      endpoint2: object.endpoint || this.state.endpoint2,
      mode2: object.mode,
    });
    if (object.endpoint)
      store.dispatch(
        endpointUpdate("right", { ...this.state.endpoint2, ...object.endpoint })
      );
  }

  onDragStart = (start) => {
    var task = JSON.parse(
      start.draggableId.slice(start.draggableId.indexOf(" "))
    );
      ...
      <Container>
        ...
        >
          <Grid item md={desktopWidth} sm={tabletWidth}>
            <FormControl component="fieldset">
              <FormLabel component="legend">
                <ToggleHeader>Optimization</ToggleHeader>
              </FormLabel>
              <RadioGroup
              ...
              >
                <FormControlLabel
                  value="None"
                  control={<Radio />}
                  label={<ToggleLabel>None</ToggleLabel>}
                />
                <FormControlLabel
                  value="VDA2C"
                  control={<Radio />}
                  label={
                    <ToggleLabel>
                      Value Decomposition Actor Critic (VDA2C)
                    </ToggleLabel>
                  }
                />
                <FormControlLabel
                  value="BO"
                  control={<Radio />}
                  label={<ToggleLabel>Bayesian Optimization (BO)</ToggleLabel>}
                />
                <FormControlLabel
                  value="SGD"
                  control={<Radio />}
                  label={
                    <ToggleLabel>Stochastic Gradient Descent (SGD)</ToggleLabel>
                  }
                />
                <FormControlLabel
                  value="MADDPG"
                  control={<Radio />}
                  label={<ToggleLabel>Multi-Agent DDPG (MADDPG)</ToggleLabel>}
                />
                <FormControlLabel
                  value="PPO"
                  control={<Radio />}
          ...
          <Grid item md={desktopWidth} sm={tabletWidth}>
            <FormControl component="fieldset">
              <FormLabel component="legend">
                <ToggleHeader>Retries</ToggleHeader>
              </FormLabel>
              <TextField
                id="outlined-number"
              ...
              />
              <FormLabel style={{ marginTop: "20px", fontSize: "20px" }}>
          ...
          <Grid item md={desktopWidth} sm={tabletWidth}>
            <FormControl component="fieldset">
              <FormLabel component="legend">
                <ToggleHeader>Date</ToggleHeader>
              </FormLabel>
          ...
          <Grid item md={desktopWidth} sm={tabletWidth}>
            <FormControl component="fieldset">
              <FormLabel component="legend">
                <ToggleHeader>Concurrency Thread Count</ToggleHeader>
              </FormLabel>
              <TextField
                id="outlined-number"
          ...
          <Grid item md={desktopWidth} sm={tabletWidth}>
            <FormControl component="fieldset">
              <FormLabel component="legend">
                <ToggleHeader>Parallel Thread Count</ToggleHeader>
              </FormLabel>
              <TextField
                id="outlined-number"
          ...
          <Grid item md={desktopWidth} sm={tabletWidth}>
            <FormControl component="fieldset">
              <FormLabel component="legend">
                <ToggleHeader>Pipe Size</ToggleHeader>
              </FormLabel>
              <TextField
                id="outlined-number"
                      ...
                      >
                        ...
                        <Grid item>
                          <Button
                            ...
                            id="sendFromLeftToRight"
                            onClick={this.onSendToRight}
                            sx={{ border: "20px" }}
                          >
                            {" "}
                            Send
